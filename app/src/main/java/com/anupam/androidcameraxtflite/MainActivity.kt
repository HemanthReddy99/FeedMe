    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if(getIntent().getExtras() != null) {
            if (getIntent().getStringExtra("sender")!=null)
                if(getIntent().getStringExtra("sender").equals("SecondActivity"))
                a = intent.getStringArrayListExtra("DataValue")
        }
        val button = findViewById<Button>(R.id.submit);
        button.setOnClickListener{
            val intent = Intent(applicationContext,SecondActivity::class.java)
            var edit = findViewById<TextView>(R.id.predictedTextView)
            itemname = edit.text.toString()
            if(!itemname.isEmpty()) {
                if(!a.contains(itemname))
                    a.add(itemname)
            }
            intent.putStringArrayListExtra("DataValue",a)
